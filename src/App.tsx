...
import { AnimationList, Keys, KeysDom } from './model/AppTypes';

const App:React.FC = ()=>{
  const [moveKeys, setMoveKeys] = useState<AnimationList>([]); //에니메이션 컴포넌트 state
  const moveKeysRef = useRef<AnimationList>([]); // effect에서 state값을바꾸어도 느리게 적용되에서 사용함
  const keysDom = useRef<KeysDom>({}); //Ref를 여러개사용하기위해 current 값에 오브젝트를 넣어
  ...
  const playSound = useCallback((sound:string) => {//소리출력하는 함수
  ...
  (e: { key: string; repeat: any; }) => {
    const changeKey:Keys = {
      //키별 계이름
      s: "C",
      d: "D",
      f: "E",
    ...
    if (e.repeat) return; //키누르는중 중복 이벤트 발생
      ...
      //입력한 키가 사용할키인지 확인
      const sound:string = changeKey[inputKey]; //계이름
      playSound(sounds[sound]); //피아노 소리 출력
      if (keysDom.current[sound] !== null) {
...
useEffect(() => {
  ...
  return (
    <Wrapper>
      <PianoBox>
        <PianoKey type="C" ref={(el:HTMLDivElement) => (keysDom.current["C"] = el)} />
        <PianoKey type="D" ref={(el:HTMLDivElement) => (keysDom.current["D"] = el)} />
        <PianoKey type="E" ref={(el:HTMLDivElement) => (keysDom.current["E"] = el)} />
        <PianoKey type="F" ref={(el:HTMLDivElement) => (keysDom.current["F"] = el)} />
        <PianoKey type="G" ref={(el:HTMLDivElement) => (keysDom.current["G"] = el)} />
        <PianoKey type="A" ref={(el:HTMLDivElement) => (keysDom.current["A"] = el)} />
        <PianoKey type="B" ref={(el:HTMLDivElement) => (keysDom.current["B"] = el)} />
        <PianoKey type="highC" ref={(el:HTMLDivElement) => (keysDom.current["highC"] = el)} />
        {moveKeys}
      </PianoBox>
    </Wrapper>
  );
...
const PianoBox = styled.main`
  ...
  z-index: 1;
`;
const PianoKey = styled.div<{type:string}>`
  width: 100px;
  height: 250px;
  border-radius: 50px;
...
const TopMovePianoKey = styled(PianoKey)<{left:number,top:number,bottom:number}>` //시작위치에서 화면 위로 이동하는 에니메이션 컴포넌트
  position: absolute;
  z-index: 10;
  left: ${({ left }) => `${left}px`};
